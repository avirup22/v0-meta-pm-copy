          ...
          <div className="p-6 flex flex-col gap-6 max-w-5xl mx-auto">
            ...
            <div className="rounded-2xl border overflow-hidden relative"
              ...
              <div className="p-5 relative">
                <div className="flex items-center gap-2.5 mb-4">
                  <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: "oklch(0.58 0.30 293)" }}>
                ...
                <div className="flex items-center gap-2 bg-white rounded-xl border px-3 py-2.5 mb-3"
                  ...
                  <button className="text-muted-foreground hover:text-primary transition-colors" aria-label="Send prompt">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {AI_SUGGESTIONS.map((s) => (
                    <button key={s} onClick={() => setAiPrompt(s)}
                      ...
                      style={{ color: "oklch(0.58 0.30 293)", borderColor: "color-mix(in oklch, oklch(0.58 0.30 293) 25%, transparent)" }}>
                      {s}
                    </button>
                  ))}
                </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                {/* RACI Matrix — dedicated page */}
                <Link href={`/projects/${slug}/documents/raci`}
                  className="rounded-xl border p-4 text-left flex flex-col gap-3 transition-all duration-150 relative overflow-hidden hover:shadow-md hover:-translate-y-0.5"
                  style={{
                    background: "color-mix(in oklch, oklch(0.65 0.20 55) 5%, white)",
                    borderColor: "color-mix(in oklch, oklch(0.65 0.20 55) 25%, transparent)",
                  }}>
                  <div className="flex items-center justify-between">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
                      style={{ background: "color-mix(in oklch, oklch(0.65 0.20 55) 14%, white)" }}>
                      <BarChart3 size={15} style={{ color: "oklch(0.65 0.20 55)" }} strokeWidth={2} />
                    </div>
                    <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded"
                      style={{ color: "oklch(0.38 0.12 55)", background: "oklch(0.92 0.08 55)" }}>
                      Interactive
                    </span>
                  </div>
                  <div className="flex-1">
                    <p className="text-xs font-bold text-foreground font-sans">RACI Matrix</p>
                    <p className="text-[10px] text-muted-foreground font-sans mt-0.5 leading-relaxed">
                      Paste your SOW and AI maps Responsible, Accountable, Consulted and Informed across all stakeholders.
                    </p>
                  </div>
                  <div className="flex items-center gap-1 text-[10px] font-semibold" style={{ color: "oklch(0.65 0.20 55)" }}>
                    Open <ArrowRight size={11} strokeWidth={2.5} />
                  </div>
                </Link>

                {/* Kickoff Presentation — dedicated page */}
                <Link href={`/projects/${slug}/documents/kickoff`}
                  className="rounded-xl border p-4 text-left flex flex-col gap-3 transition-all duration-150 relative overflow-hidden hover:shadow-md hover:-translate-y-0.5"
                  style={{
                    background: "color-mix(in oklch, oklch(0.70 0.20 35) 5%, white)",
                    borderColor: "color-mix(in oklch, oklch(0.70 0.20 35) 25%, transparent)",
                  }}>
                  <div className="flex items-center justify-between">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
                      style={{ background: "color-mix(in oklch, oklch(0.70 0.20 35) 14%, white)" }}>
                      <Presentation size={15} style={{ color: "oklch(0.70 0.20 35)" }} strokeWidth={2} />
                    </div>
                    <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded"
                      style={{ color: "oklch(0.36 0.14 30)", background: "oklch(0.94 0.08 35)" }}>
                      PPTX
                    </span>
                  </div>
                  <div className="flex-1">
                    <p className="text-xs font-bold text-foreground font-sans">Kickoff Presentation</p>
                    <p className="text-[10px] text-muted-foreground font-sans mt-0.5 leading-relaxed">
                      Executive-ready PPT with project overview, objectives, team, timeline, and risks.
                    </p>
                  </div>
                  <div className="flex items-center gap-1 text-[10px] font-semibold" style={{ color: "oklch(0.70 0.20 35)" }}>
                    Open <ArrowRight size={11} strokeWidth={2.5} />
                  </div>
                </Link>

                {/* Standard AI-generated templates */}
                {DOC_TEMPLATES.map((tmpl) => {
                  const isSelected = selectedTemplate === tmpl.id
                  return (
                    <button key={tmpl.id}
                      onClick={() => setSelectedTemplate(isSelected ? null : tmpl.id)}
                      className="rounded-xl border p-4 text-left flex flex-col gap-3 transition-all duration-150 relative overflow-hidden"
                      style={{
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded"
                          style={TYPE_BADGE[tmpl.type] ? { color: TYPE_BADGE[tmpl.type].color, background: TYPE_BADGE[tmpl.type].bg } : {}}>
            ...
            {selectedTemplate && (
              <div className="sticky bottom-0 bg-background/90 backdrop-blur-sm border-t border-border -mx-6 px-6 py-4 flex items-center justify-between gap-4">
                ...
                <button onClick={handleGenerate} disabled={isGenerating}
                  ...
                  {isGenerating ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} strokeWidth={2.5} />}
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden">
              ...
              {library.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <button onClick={() => setActiveTab("generate")} className="text-xs font-semibold text-primary hover:underline font-sans">
                ...
                <ul className="divide-y divide-border">
                  {library.map((doc) => {
                    ...
                    return (
                      <li key={doc.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/40 transition-colors group">
